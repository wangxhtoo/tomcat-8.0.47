...
public abstract class AbstractHttp11Processor<S> extends AbstractProcessor<S> {
	 ...
	 * like a keep-alive connection - i.e. one where there may not be any data to
	 * process. The initial value of this flag on entering the process method is
	 * different for connectors that use polling (NIO / APR - data is always
	 * expected) compared to those that use blocking (BIO - data is only expected if
	 * the connection isn't in the keep-alive state).
	 */
	...
	/**
	 * List of MIMES which could be gzipped Note: This is not spelled correctly but
	...
	/**
	 * Set compressible mime-type list List contains users agents separated by ',' :
	 *
	...
	/**
	 * Set the maximum number of Keep-Alive requests to honor. This is to safeguard
	 * from DoS attacks. Setting to a negative value disables the check.
	 */
	...
	/**
	 * Processors (currently only HTTP BIO) may elect to disable HTTP keep-alive in
	 * some circumstances. This method allows the processor implementation to
	 ...
	 *
	 * @return <code>true</code> if the processor should break out of the processing
	 ...
	 *
	 * @param socketWrapper Socket from which the HTTP requests will be read and the
	 *                      HTTP responses will be written.
	 *
	...
	public SocketState process(SocketWrapper<S> socketWrapper) throws IOException {
		...
		while (!getErrorState().isError() && keepAlive && !comet && !isAsync() && upgradeToken == null
				&& !endpoint.isPaused()) {
	...
	/**
	 * Connector implementation specific request preparation. Ideally, this will go
	 * away in the future.
	 */
	...
	public SocketState upgradeDispatch(SocketStatus status) throws IOException {
		...
		throw new IllegalStateException(sm.getString("http11Processor.upgrade"));
	}
	...
	/**
	 * Provides a mechanism for those connector implementations (currently only NIO)
	 * that need to reset timeouts from Async timeouts to standard HTTP timeouts
	 * once async processing completes.
	 */
	protected abstract void resetTimeouts();

	/**
	 * Provides a mechanism for those connectors (currently only NIO) that need that
	 * need to set comet timeouts.
	 */
	 ...
	 * processing (e.g. sendfile handling) that may have an impact on whether or not
	 * the keep-alive loop should be broken.
	 * 
