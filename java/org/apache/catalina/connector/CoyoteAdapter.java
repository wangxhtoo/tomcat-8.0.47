...
public class CoyoteAdapter implements Adapter {
	...
	public void service(org.apache.coyote.Request req, org.apache.coyote.Response res) throws Exception {
		...
		if (request == null) {
			...
			// Create objects
			// 1. 通过 Connector 创建
			// org.apache.catalina.connector.Request对象,org.apache.catalina.connectorResponse
			// 对象 这里的 Request, Response 实现了 ServletRequest/ServletResponse, 并且在内部属性中拥有
			// org.apache.coyote.Request/Response

			// 2. 这里的 Request 或实现 ServletRequest接口, 并且会传递到下游容器中 (PS: 在创建 Connector 时会构建一个
		...
		try {
			...
			// request parameters
			// 8. 在 RequestInfo 里面设置对应的 ThreadName (PS: 可以看到 这里 ThreadLocal 只有对对应 get,
			// 但没有对应的 remove, 为什么呢? 因为这里存储 Tomcat 工作线程池的领地, 工作线程池 never stop, 除非停止 Tomcat,
			// 并且这里缓存的数据的大小也是非常小的, 也不会与其他的 WebappClassLoader/WebappClassLoader 任何挂钩)
			req.getRequestProcessor().setWorkerThreadName(THREAD_NAME.get());
			// 9. 下面的 postParseRequest 是用来处理请求映射 (获取 host, context, wrapper, URI 后面的参数的解析,
			// sessionId )
			...
			if (postParseSuccess) {
				...
				// Calling the container
				// 10. 开始调用 Tomcat 的容器, 首先调用 StandardEngine 容器中的管道PipeLine 中的第一个 Valve, 传入
				// connector.Request 与 connector.Response
				connector.getService().getContainer().getPipeline().getFirst().invoke(request, response);
	 ...
	 *                          processing headers
	 * @throws ServletException If the supported methods of the target servlet can
	 *                          not be determined
	 */
	@SuppressWarnings("deprecation")
